...
#'
#' @return plots are generated and saved in /output/met/barplot/
#' @export
#'
#' @examples
#'
...
event_timeseries_precip <- function(var_in,
  ...
  input_Parameters <- openxlsx::read.xlsx(var_in, sheet = "precip_barplots")
  ...
  ls_par <- if(user_units == "English") {
    lapply(ls_par, function(x) {x$totprcp <- x$totprcp / 25.4; x})
    } else { ls_par } # mm to in
  ...
  for(i in 1:length(ls_par)) {
      ...
      if(flip == FALSE) {
      # basic plot
      ylabel <- if(user_units == "English") {'Daily Precipitation (in)'} else { 'Daily Precipitation (mm)'}
      ...
      if(flip == FALSE) {
        ...
        ylabel <- if(user_units == "English") {'Max Precipitation Intensity (in/hour)'} else { 'Max Precipitation Intensity (mm/hour)'}
        # basic plot
